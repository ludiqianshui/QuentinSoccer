...
function LoadTitleLeft() {
    ...
    titleLeftHtml += arrLeague[4] + " " + arrLeague[lang + 1] + " 赛程积分&nbsp;&nbsp;";  //标题
    titleLeftHtml += "<a href='javascript:void(0)' onclick='miniopen(\"/ClientErr/InfoErr.aspx?SclassID=" + arrLeague[0] + "&MatchSeason=" + arrLeague[4] + "&CurrRound=" + selectRound + "\")'  class='btn'>报错</a>"; //报错
...
function showRound() {
    ...
    html += "<td width='66' rowspan='2' bgcolor='" + arrLeague[5] + "' ><div style='margin: 0 auto;width:100%;'  class='write'><strong>" + arrLeague[9 + lang] + "<br />第 " + selectRound + " 轮</strong></div></td>";
...
function showSche() {
    ...
    for (var i = 0; i < jh["R_" + selectRound].length; i++) {
        ...
        tr1.appendChild(creatTd(null, null, h_redCard + "<a href='/cn/team/Summary/" + arrOneRecord[4] + ".html' target='_blank'>" + teamHelper["T_" + arrOneRecord[4]][1 + lang] + "</a><sup>[" + arrOneRecord[8] + "]</sup>"));  //主队

        tr1.appendChild(creatTd(null, null, "<div class='redf' align='center'><a href='http://bf.win007.com/detail/" + arrOneRecord[0] + "cn.htm'  target='_blank'><strong>" + showScheScore(arrOneRecord[6]) + "</strong></a></div>")); //比分
        var g_redCard = "";
        ...
        tr1.appendChild(creatTd(null, null, "<a href='/cn/team/Summary/" + arrOneRecord[5] + ".html' target='_blank'>" + teamHelper["T_" + arrOneRecord[5]][1 + lang] + "</a><sup>[" + arrOneRecord[9] + "]</sup>" + g_redCard)); //客队
        if (i % 2 == 0) {
        ...
        if (arrOneRecord[20] != "") {
            try {
                ...
                if (gex4[0] != "")
                    explainTemp += gex4[0].replace(",", "分钟[") + "],";
                if (gex4[1] != "")
                    explainTemp += "两回合[" + gex4[1] + "],";
                if (gex4[2] != "")
                    explainTemp += gex4[2].replace("1,", "120分钟[").replace("2,", "加时[") + "],";
                if (gex4[3] != "")
                    explainTemp += "点球[" + gex4[3] + "],";
                if (gex4[4] == "1")
                    explainTemp += teamHelper["T_" + arrOneRecord[4]][1 + lang] + "赢";
                else if (gex4[4] == "2")
                    explainTemp += teamHelper["T_" + arrOneRecord[5]][1 + lang] + "赢";
...
function showFour(scheid, x, o, y, d) {
    var str = "";
    if (x != 0)
        str += "<a href='/analysis/" + scheid + ".htm' target='_blank' >[析]</a>";
    if (o != 0)
        str += "<a href='http://1x2.win007.com/oddslist/" + scheid + ".htm' target='_blank' >[欧]</a>";
    if (x + o + y + d > 2)
        str += "<br/>";
    if (y != 0)
        str += "<a href='http://vip.win007.com/AsianOdds_n.aspx?id=" + scheid + "' target='_blank' >[亚]</a>";
    if (d != 0)
...
function showTotalScore() {
    ...
    arrData.push("<table id='div_Table1' width='790' border='1' cellpadding='2' cellspacing='0' style='font-size:small'  bgcolor='#FFFFFF' class='tdlink'><tr bgcolor='#D1E2F5' align='center' style='font-weight:bold;'><td width='30'>排名</td><td  height='18'  >球队名称</td><td width='32' >赛</td><td width='32' >胜</td><td width='32'>平</td><td width='32'>负</td><td width='32'>得</td><td width='32' >失</td><td width='32' >净</td><td width='50'>胜%</td><td width='50'>平%</td><td width='50'>负%</td><td width='28'>均得</td><td width='28'>均失</td><td width='28' >积分</td><td width='100' >近六轮</td></tr>");
...
function changeTeam() {
    ...
    window.location.href = "/cn/LeaTeam.aspx?TeamID=" + drop1Val + "&sclassID=" + arrLeague[0] + "&matchSeason=" + selectSeason + "&lang=" + lang;
}

function changeSeason() {
    var seasonList = document.getElementById("seasonList");
    var season = seasonList.options[seasonList.selectedIndex].value;
    var ul = "/cn/League/" + season + "/" + arrLeague[0] + ".html";
