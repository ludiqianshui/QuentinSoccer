...
class prosoccer_data_dump(object):
    ...
    ProsoccerGameInfoDic = {'GameTime': '', 'GameLeague':'', 'GameHost':'', 'GameGuest':'', 
...
# get content of tr(each tr is a each game info) and td (each td is a item of the game info)
        for tr_tag in tabel_root:
            ...
            if tr_tag.tag == "tr" and (tr_class_name == "f1" or tr_class_name == "f2") :
                ...
                # get game time
                self.ProsoccerGameInfoDic['GameTime'] = td_tag_list[0].text
                # get game type
                self.ProsoccerGameInfoDic['GameLeague'] = str(td_tag_list[1].getchildren()[0].tail).replace('\xc2\xa0','')
                # get GameHost
                self.ProsoccerGameInfoDic['GameHost'] = str(td_tag_list[2].getchildren()[0].text).split('-')[0].replace(' ','')
                # get GameGuest
                self.ProsoccerGameInfoDic['GameGuest'] = str(td_tag_list[2].getchildren()[0].text).split('-')[1].replace(' ','')
                ...
                if td_tag_list[14].text == ' - ':
                    ...
                    self.ProsoccerGameInfoDic['FinalScore'] = td_tag_list[14].text
               
                # get tips 
                tips_value = td_tag_list[6].getchildren()[0].tail
                if tips_value == "emp":
                    self.ProsoccerGameInfoDic['PredictTIPS'] = "NOBET"
                elif tips_value[0] == "p":
                    self.ProsoccerGameInfoDic['PredictTIPS'] = str(tips_value[1:]).upper()
                ...
                # get odds
                self.ProsoccerGameInfoDic['ODD_1'] = str(td_tag_list[7].text).replace('\xc2\xa0','')
                self.ProsoccerGameInfoDic['ODD_X'] = str(td_tag_list[8].text).replace('\xc2\xa0','')
                self.ProsoccerGameInfoDic['ODD_2'] = str(td_tag_list[9].text).replace('\xc2\xa0','')
             
                # get predict 2.5 Goals 
                self.ProsoccerGameInfoDic['PredictPossibilityUnder2.5'] = td_tag_list[12].text 
                self.ProsoccerGameInfoDic['PredictPossibilityOver2.5'] = td_tag_list[13].text 

                # if the game finished      
                if self.FlagGameFinished == True:
                    # get predict possibility win
                    self.ProsoccerGameInfoDic['PredictPossibility_1'] = td_tag_list[3].text
                    # get predict possibility draw
                    self.ProsoccerGameInfoDic['PredictPossibility_X'] = td_tag_list[4].text
                    # get predict possibility lose
                    self.ProsoccerGameInfoDic['PredictPossibility_2'] = td_tag_list[5].text
                    # get predict score             
                    if td_tag_list[10].getchildren() == None:
                        self.ProsoccerGameInfoDic['PredictScore_1'] = td_tag_list[10].text
                    if len(td_tag_list[10].getchildren()) == 1 and td_tag_list[10].text  == None:
                        self.ProsoccerGameInfoDic['PredictScore_1'] = td_tag_list[10].getchildren()[0].text + td_tag_list[10].getchildren()[0].tail
                    elif len(td_tag_list[10].getchildren()) == 1 and td_tag_list[10].text  != None:
                        self.ProsoccerGameInfoDic['PredictScore_1'] = td_tag_list[10].text + td_tag_list[10].getchildren()[0].text 
                    if len(td_tag_list[10].getchildren()) == 2:
                        self.ProsoccerGameInfoDic['PredictScore_1'] = td_tag_list[10].getchildren()[0].text +"-" + td_tag_list[10].getchildren()[1].text
                    
                    if td_tag_list[11].getchildren() == None:
                        self.ProsoccerGameInfoDic['PredictScore_2'] = td_tag_list[11].text
                    if len(td_tag_list[11].getchildren()) == 1 and td_tag_list[11].text  == None:
                        self.ProsoccerGameInfoDic['PredictScore_2'] = td_tag_list[11].getchildren()[0].text + td_tag_list[11].getchildren()[0].tail
                    elif len(td_tag_list[11].getchildren()) == 1 and td_tag_list[11].text  != None:
                        self.ProsoccerGameInfoDic['PredictScore_2'] = td_tag_list[11].text + td_tag_list[11].getchildren()[0].text 
                    if len(td_tag_list[11].getchildren()) == 2:
                        self.ProsoccerGameInfoDic['PredictScore_2'] = td_tag_list[11].getchildren()[0].text +"-" + td_tag_list[11].getchildren()[1].text
                ...
                if self.FlagGameFinished == False:
                    ...
                    self.ProsoccerGameInfoDic['PredictPossibility_1'] = int(number_1st_digit + number_2st_digit)
                    # get predict possibility draw
                    ...
                    self.ProsoccerGameInfoDic['PredictPossibility_X'] = int(number_1st_digit + number_2st_digit)
                    # get predict possibility lose
                    ...
                    self.ProsoccerGameInfoDic['PredictPossibility_2'] = int(number_1st_digit + number_2st_digit)
                    # get predict score             
                    self.ProsoccerGameInfoDic['PredictScore_1'] = td_tag_list[10].getchildren()[0].text +"-" + td_tag_list[10].getchildren()[1].text
                    self.ProsoccerGameInfoDic['PredictScore_2'] = td_tag_list[11].getchildren()[0].text +"-" + td_tag_list[11].getchildren()[1].text
                
                print self.ProsoccerGameInfoDic
        return True
